...
export const postBooking = async (req: Request, res: Response) => {
  ...
  const flightResponse = await fetch(
    backendReportUrl + "/flight/id/?id=" + flightId + "&currency=" + currency
  );
  ...
  let booking: Booking = {
    ...
    price: bookingPrice,
    status:
      flightResponseData.company === "Temps Partiel" ? "confirmed" : "pending",
  };
  ...
  if (flightResponseData.remainingSeats === 0)
    throw new handledError(404, "Flight is complete");
  if (!flightResponseData.menuVege && booking.vege)
    throw new handledError(404, "Vegetarian menu not available on this flight");
  ...
  const putRemainingSeatResponse = await fetch(
    databaseAccesUrl + "/flight/remainingSeats",
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
...
export const getBookingById = async (req: Request, res: Response) => {
  ...
  const getBookingByIdResponse = await fetch(
    databaseAccesUrl + "/booking/id/?id=" + id
  );
...
export const cancelBooking = async (req: Request, res: Response) => {
  ...
  const cancelBookingResponse = await fetch(
    databaseAccesUrl + "/booking/cancel",
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!cancelBookingResponse.ok) {
    const resJson = await cancelBookingResponse.json();
